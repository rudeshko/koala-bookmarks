        ...
        for(row = 0; row < 4; row++){
            ...
            for(i = count; i < count + 4; i++){
                if(bookmarks[i]['url'] !== "" && bookmarks[i]['name'] !== ""){
                    var add = "";
                    add += "<div class='cell'>";
                    add += "<a class='dummy' href='" + bookmarks[i]['url'] + "' target='_blank'>";
                        add += "<div class='link'></div>";
                        
                        add += "<div class='circle'>";
                            add += "<div class='icon'>";
                            add += "<img src='http://www.google.com/s2/favicons?domain=" + bookmarks[i]['url'] + "' />";
                            add += "</div>";
                        add += "</div>";
                    ...
                    rowAdd += "<div class='cell'><a href='./options.html#" + (i + 1) + "' target='_blank'><div class='circle add'>+</div></a></div>";
                }
            }
            rowAdd += "</div>";
            count += 4;
...
setTimeout(function(){
    $(document).on("keyup", "body", function(e){
        ...
        if(key >= 1 && key <= 9){
            var $tab = $("div.cell").eq(key - 1).children("a")[0];
            if($tab !== undefined){
                $tab.click();
            }
        }
