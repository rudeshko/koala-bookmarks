function updateBookmarks(bookmarks){
    ...
    for(var i = 0; i < 16; i++){
        if(bookmarks[i]["url"]){
            $("div.selection-row[data-id=" + (i + 1) + "]").css("background-image", "url('http://www.google.com/s2/favicons?domain=" + bookmarks[i]["url"] + "')").attr("data-name", bookmarks[i]["name"]).attr("data-url", bookmarks[i]["url"]);
        }else{
            $("div.selection-row[data-id=" + (i + 1) + "]").css("background-image", "");
        }
    }
}
...
$(document).ready(function(){
    chrome.storage.sync.get("bookmarks", function(obj){
        ...
        $("div.selection-row").click(function(){
            ...
            $("input[name=name]").val($(this).attr("data-name")).focus();
